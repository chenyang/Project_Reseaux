...
void mail_to_str(const mail *m, char *s) {sprintf(s,"mail|%s|%s|%s|%s|%s", m->dst_nick, m->src_nick, m->date, m->object, m->message);}
...
list* delete_position(list* l, int i){
    ...
    if(i==1){
        ...
        free(h);
        h=n;
        //printf("The mail is correcly deleted\n");
        return n;
    }else{//i!=1
    list* p=get_position(l, i);
    list* pre=get_position(l, i-1);
    if(p!=NULL&&pre!=NULL){
        p=pre->next;
        pre->next=p->next;
        free(p);
        //printf("The mail is correcly deleted\n");
        return l;
        }else{//printf("We can not delete the mail which does not exists\n");
        return l;
        }
    }
}
